...
import { ALL_METADATA, SPECIAL_COLUMNS } from "../../constants";
        ...
        const total = sum(
            data.map((d) => {
                if (
                    ...
                    !isNaN(Number(d[others.aggregationColumn]))
                ) {
...
const findMerged = (
    ...
    if (data) {
        const finalColumns: Array<Array<Column>> = [];
                        ...
                        if (specific.indexOf(key) !== -1) {
                            return {
                                ...
                                y: Object.keys(groupedByCategory)
                                    .sort()
                                    .map((k) => {
                                        const current = currentValues.filter(
                                            (val) =>
            ...
            const columns = orderBy(
                x.map((c: any) => {
                    return {
                        id: c,
                        name:
                            ALL_METADATA[c] || options.metadata?.[c]?.name || c,
                    };
                }),
                "name",
                "asc"
